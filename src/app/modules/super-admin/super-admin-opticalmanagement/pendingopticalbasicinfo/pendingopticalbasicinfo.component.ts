...
@Component({
  selector: 'app-pendingopticalbasicinfo',
  templateUrl: './pendingopticalbasicinfo.component.html',
  styleUrls: ['./pendingopticalbasicinfo.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class PendingopticalbasicinfoComponent implements OnInit {
  ...
  approveOrRejectOptical(action: any) {
    let reqData = {
      verify_status: action,
      approved_or_rejected_by: this.superAdminId,
      doctor_portal_id: this.opticalId,
    };
    ...
    this.labimagingdentaloptical.approveOrRejectLabimagingdentaloptical(reqData).subscribe(
      (res) => {
        ...
        if (response.status) {
          ...
          if (action === "APPROVED") {
            this.route.navigate([
              "/super-admin/individualoptical/permission",
              this.opticalId,
            ]);
          } else {
