...
export class PendingimagingbasicinfoComponent implements OnInit {
  ...
  getImagingDetails() {
    ...
    this.labimagingdentaloptical.getLabimagingdentalopticalDetails(reqData).subscribe((res) => {
      ...
      let team_id = response?.data[0]?.team;
      ...
      (res) => {
        ...
        if (response.status) {
          ...
          if (action === "APPROVED") {
            ...
            this.route.navigate(["/super-admin/individuallaboratory-imaging"]);
          }
        }
      },
      (err) => {
  ...
  getLocations(data: any) {
    ...
    // this.service.getLocations(this.imagingId).subscribe((res) => {
      let reqdata={
        portal_user_id:this.imagingId,
        type: "Laboratory-Imaging"
      }
