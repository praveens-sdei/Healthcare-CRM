...
@Component({
  selector: 'app-pendingdentalbasicinfo',
  templateUrl: './pendingdentalbasicinfo.component.html',
  styleUrls: ['./pendingdentalbasicinfo.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class PendingdentalbasicinfoComponent implements OnInit {
  ...
  approveOrRejectLab(action: any) {
    let reqData = {
      ...
      doctor_portal_id: this.dentalId,
    };
    ...
    this.labimagingdentaloptical.approveOrRejectLabimagingdentaloptical(reqData).subscribe(
      (res) => {
        ...
        if (response.status) {
          ...
          if (action === "APPROVED") {
            this.route.navigate([
              "/super-admin/individualdental/permission",
              this.dentalId,
            ]);
          } else {
            this.route.navigate(["/super-admin/individualdental"]);
          }
        }
      },
      (err) => {
