...
export class VillageComponent implements OnInit {
  ...
  constructor(
    ...
    this.addVillageDetails = this.fb.group({
      _id: [""],
  ...
  get f() {
    return this.addVillageDetails.controls;
  }
  ...
  onSortData(column:any) {
    ...
    this.getAllVillageList(`${this.sortColumn}:${this.sortOrder}`);
  }
  ...
  checkInnerPermission(){
    ...
    if(checkData){
      if(checkData.isChildKey == true){
        ...
        if (checkSubmenu.hasOwnProperty("village")) {
          this.innerMenuPremission = checkSubmenu['village'].inner_menu;
  ...
  openVerticallyCenteredAddvaccination(addvaccinationcontent: any) {
    this.countryList = []
    this.villageDetails = {};
    this.addVillageDetails.get("name").patchValue("")
  ...
  getRegionList(countryID: any) {
    ...
    this._superAdminService.getRegionListByCountryId(countryID).subscribe({
      next: (res) => {
        ...
        if(this.villageDetails)
        {
          this.addVillageDetails.get("region_id").patchValue(this.villageDetails?.regionData?._id)
        }
      },
      error: (err) => {
        console.log(err);
  ...
  getProvienceList(regionID: any) {
    ...
    this._superAdminService.getProvinceListByRegionId(regionID).subscribe({
      next: (res) => {
        ...
        if(this.villageDetails)
        {
          this.addVillageDetails.get("province_id").patchValue(this.villageDetails?.provinceData?._id)
        }
      },
      error: (err) => {
        console.log(err);
  ...
  getDepartmentList(provinceID: any) {
    ...
    this._superAdminService.getDepartmentListByProvinceId(provinceID).subscribe({
      next: (res) => {
        ...
        if(this.villageDetails)
        {
          this.addVillageDetails.get("department_id").patchValue(this.villageDetails?.departmentData?._id)
        }
      },
      error: (err) => {
        console.log(err);
  ...
  closePopup() {
    ...
    this.addVillageDetails.reset();

  }
  handleaddProvince() {
    this.isSubmitted = true;
    if (this.addVillageDetails.invalid) {
    ...
    this._superAdminService
      .addVillage(this.addVillageDetails.value)
      .subscribe(
        (res) => {
          try {
            let data = this._coreService.decryptObjectData({ data: res });
  ...
  getAllVillageList(sort:any="") {
    ...
    this._superAdminService.VillageMasterLists(params).subscribe((res) => {
      let data = this._coreService.decryptObjectData({ data: res });
  ...
  updateVillage() {
    ...
    this.loader.start();
    this._superAdminService
      .updateVillageApi(this.addVillageDetails.value)
      .subscribe((res: any) => {
        let encryptedData = { data: res };
        let response = this._coreService.decryptObjectData(encryptedData);
...
this._superAdminService.Country_dropdownLists().subscribe(async(res) => {
  ...
  const countryListEdit = CountryData?.body?.countrylist;
  countryListEdit.forEach(country => {
    const exists = this.countryListEdit.findIndex(item => item.label === country?.name && item.value === country?._id) !== -1;
    if (!exists) {
      this.countryListEdit.push({
        label: country?.name,
        value: country?._id,
      });
    }
  if(this.villageDetails)
  {
    this.addVillageDetails.get("country_id").patchValue(this.villageDetails?.countryData._id)
  }
  });
...
await this.getDepartmentList(data?.departmentData?.province_id)
  this.addVillageDetails.patchValue({
    _id: data?._id,
    name: data?.name,
...
this.modalService.open(editvaccinationcontent, {
  ...
  windowClass: "master_modal edit_lab",
   ...
   exportVaccination() {
    ...
    this.pageSize = 0;
    this._superAdminService
      .villageexcelListforexport(this.page, this.pageSize, this.searchKey)
      .subscribe((res) => {
        let result = this._coreService.decryptObjectData({ data: res });
...
excleSubmit() {
  ...
  // uploadExcelMedicine
  this._superAdminService.uploadExcelVillageList(formData).subscribe(
    (res: any) => {
      let encryptedData = { data: res };
      let response = this._coreService.decryptObjectData(encryptedData);
...
downLoadExcel() {
  ...
  link.setAttribute("href", "assets/doc/villageList.xlsx");
  link.setAttribute("download", `villageList.xlsx`);
