...
@Component({
  selector: 'app-insurance-communication',
  templateUrl: './insurance-communication.component.html',
  styleUrls: ['./insurance-communication.component.scss'],
  encapsulation:ViewEncapsulation.None
})
export class InsuranceCommunicationComponent implements OnInit {
  ...
  private getAllStaff() {
    ...
    this._insuranceService.getInsuranceChatUser(params).subscribe((res: any) => {
      ...
      if (response) {
        for (const staff of response?.data?.data) {
          data.push({
            username: staff.user_name ?  staff.user_name : "",
  ...
  getRoomList(chatId: any = '') {
    ...
    this._insuranceService.getRoomlistService(params).subscribe((res: any) => {
  ...
  handleroute(data: any, openDiv: boolean) {
    ...
    if (data?.isGroupChat) {
      ...
      : (data?.senderDetails?.profile_picture || '../../../../assets/img/default_user.png')
    }
  ...
  getAllmessages() {
    ...
    this._insuranceService.getAllMessagesService(params).subscribe((res) => {
      let encryptedData = { data: res };
      let response = this._coreService.decryptObjectData(encryptedData);
      if (response?.status) {
  ...
  deleteAllmessages() {
    ...
    this._insuranceService.clearAllMessages(params).subscribe((res: any) => {
      let encryptedData = { data: res };
      let response = this._coreService.decryptObjectData(encryptedData);
      if(response?.status){
        this.closePopup();
  ...
  deleteSingleMessage(){
    ...
    this._insuranceService.clearSingleMessages(params).subscribe((res: any) => {
      let encryptedData = { data: res };
      let response = this._coreService.decryptObjectData(encryptedData);
      if(response.status){
        this.closePopup();
