...
import { Component, Input, OnInit, Output, ViewChild, ViewEncapsulation, EventEmitter, TemplateRef } from '@angular/core';
...
export class CommoninformationComponent implements OnInit {
  ...
  fourthFormGroup = this._formBuilder.group({
    ...
    prescriberMName: new FormControl("", [Validators.required]),
    ...
    this.pharmacyService.medicineClaimDetails(this.selectclaimid).subscribe({
      next: async (res) => {
        ...
        if (result.status) {
          ...
          //   const param = {
          //     pharmacyId: claimData?.deliverCenterInfo?.deliverCenter,
  ...
  //     console.log('select2updatekey', key);

  //     // this[key] = event.value;
  //     console.log('select2updateEvent', event);
  ...
  //     console.log('select2updateEventValue', event.value);
      ...
      this.patientService.allRole(this.pharmacy_id).subscribe((res) => {
        ...
        if (result?.status) {
          ...
          if (this.editClaimData != '') {
            ...
            if (this.editClaimData?.deliverCenterInfo?.deliverTitleId != undefined) {
              const param = {
                pharmacyId: this.editClaimData?.deliverCenterInfo?.deliverCenter,
                staffRoleId: this.editClaimData?.deliverCenterInfo?.deliverTitleId,
              };
              // console.log(param, "check param");
              ...
              this.pharmacyService.getPharmacyStaffName(param).subscribe({
                next: async (res) => {
                  ...
                  if (result.status) {
                    const checkstaff = this.staffRole.find(
                      (obj) =>
                        obj._id === this.editClaimData?.deliverCenterInfo?.deliverTitleId
                    );
                    // console.log(checkstaff, "event");
                    this.staffTitle = checkstaff.name;
                    this.staffTitleId =
                      this.editClaimData?.deliverCenterInfo?.deliverTitleId;


                    // this.deliverTitleselected = this.staffTitle
                    this.deliverTitleselected = this.staffTitleId;
                    this.prescriberSpecialitydefault = this.editClaimData?.prescriberCenterInfo?.prescriberSpecialty;
                  } else {
                    this.staffTitle = "";
                    this.staffTitleId = "";
                  }
              ...
              this.firstFormGroup.patchValue({ service: this.editClaimData?.service });
            }
          }
      ...
      this.pharmacyService.getPharmacyStaffName(param).subscribe({
        next: async (res) => {
          ...
          if (result.status) {
            ...
            this.staffdata1 = []
            result?.data.map((curentval) => {
              if (this.staffdata1.indexOf({
                label: curentval?.staff_name,
                value: curentval?.staff_name,
              }) == -1) {
                this.staffdata1.push({
                  label: curentval?.staff_name,
                  value: curentval?.staff_name,
                });
              }
      ...
      next: async (res) => {
        ...
        if (result.data) {
          ...
          if (this.loggedInRole == "INSURANCE_ADMIN") {
            ...
            formVal.created_by = "INSURANCE_STAFF"
          }

          formVal.loggedInPatientId = "";
          formVal.loggedInInsuranceId = this.loggedId;
  ...
  saveStepTwo(formVal: any, stepper: MatStepper) {
    ...
    let finalReqData = {
      ...
      loggedInInsuranceId: this.loggedId,
    };
...
console.log("formVal___________",formVal);
    ...
    this.pharmacyService.saveStepFive(finalReqData).subscribe({
      next: (res) => {
        ...
        if (result.status) {
          ...
          // this.insuranceId = insuranceId?._id;
  ...
  public redirectToMainStepper() {
    ...
    if (
      ...
      this.router.createUrlTree(["/patient/medicinclaims"]);
    } else {
      this.mstepper.next();
    }
  }
  ...
  listAllPharmacy() {
    ...
    this.superAdminService.listPharmacy("").subscribe((res) => {
      let response = this.coreService.decryptObjectData(res);
      // console.log(response, "check data hospital");
      if (response.status) {
        let data = []
        ...
        if (this.editClaimData != '') {
          this.fourthFormGroup.patchValue({
            ...
            prescriberFName:
              this.editClaimData?.prescriberCenterInfo?.prescriberFirstName,
            prescriberLName:
              this.editClaimData?.prescriberCenterInfo?.prescriberLastName,
            prescriberMName:
              this.editClaimData?.prescriberCenterInfo?.prescriberMiddleName,
