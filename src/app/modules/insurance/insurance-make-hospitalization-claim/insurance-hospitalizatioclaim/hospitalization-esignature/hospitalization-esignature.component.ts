...
@Component({
  selector: 'app-hospitalization-esignature',
  templateUrl: './hospitalization-esignature.component.html',
  styleUrls: ['./hospitalization-esignature.component.scss']
})
export class HospitalizationEsignatureComponent implements OnInit {
    ...
    this.pharmacyPlanService.finalSubmitClaimDoctor(data).subscribe({
      next: async (res) => {
        ...
        if (result.status) {
          ...
          this.closePopUp();
          this.router.navigate(["/insurance/make-hospitalization-claim"]);
        } else {
