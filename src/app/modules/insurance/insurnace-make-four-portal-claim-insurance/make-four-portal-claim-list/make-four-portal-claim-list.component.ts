...
@Component({
  selector: 'app-make-four-portal-claim-list',
  templateUrl: './make-four-portal-claim-list.component.html',
  styleUrls: ['./make-four-portal-claim-list.component.scss'],
...
export class MakeFourPortalClaimListComponent implements OnInit {
  ...
  getClaimList() {
    let reqData = {
      ...
      pharmacyIds: this.patientId,
      status: this.claimStatus,
      page: this.page,
      limit: this.pageSize,
      insuranceIds: this.patientId,
      patientId: this.patientId,
      claimType: this.loggedType + "-order"
    };
    this.pharmacyPlanService.MakeInsuranceAdminFourPortalList(reqData).subscribe((res) => {
  ...
  editclaim(claim_id, type = '', preAuthReclaimId = '') {
    ...
    this.router.navigate([`insurance/make-order-claim/submitclaim/${this.loggedType}`], {
  ...
  goToNewEdit(claimId) {
    this.router.navigate([`/insurance/make-order-claim/details/${claimId}/${this.loggedType}`])
  }
