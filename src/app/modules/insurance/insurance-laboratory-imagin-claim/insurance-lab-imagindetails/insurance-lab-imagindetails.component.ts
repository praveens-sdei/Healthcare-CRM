...
@Component({
  selector: 'app-insurance-lab-imagindetails',
  templateUrl: './insurance-lab-imagindetails.component.html',
  styleUrls: ['./insurance-lab-imagindetails.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class InsuranceLabImagindetailsComponent implements OnInit {
  ...
  checkInnerPermission() {
    ...
    if (checkData) {
      if (checkData.isChildKey == true) {
        ...
        if (checkSubmenu.hasOwnProperty("imaging_claim")) {
          this.innerMenuPremission = checkSubmenu['imaging_claim'].inner_menu;
  ...
  handleSend(status) {
    ...
    this.service.addClaimForInsuranceStaff(reqData).subscribe((res) => {
      ...
      if (response.status) {
        ...
        this.router.navigate(["/insurance/imaging-claim"]);
      }
    });
  }
  ...
  routeToClaimList() {
    this.router.navigate(["/imaging-claim/medicines"]);
  }
  ...
  checkLoginUser(val, medicineId, actualColumn = '') {
    ...
    if (actualColumn == this.staffRoleID) {
      ...
      for (let i = 0; i < val.length; i++) {
        ...
        // let roleId = currentObject.staffadmininfo[0].role;
        console.log(roleId, "check staff roleaaaa id", roleId.indexOf(targetObject.roleName));
  ...
  doughnutChart1(totalcarelimit: any, subscriberId: any, plan_validity: any, type: any) {
    ...
    this.service.categoryclaimGraph(reqData).subscribe({
      next: async (res) => {
        ...
        console.log(this.total_assured_service_amount, "this.total_assured_service_amount")
