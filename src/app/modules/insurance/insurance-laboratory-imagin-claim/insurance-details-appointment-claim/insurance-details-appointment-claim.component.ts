...
@Component({
  selector: 'app-insurance-details-appointment-claim',
  templateUrl: './insurance-details-appointment-claim.component.html',
  styleUrls: ['./insurance-details-appointment-claim.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class InsuranceDetailsAppointmentClaimComponent implements OnInit {
  ...
  family_total_insured_amount: { totallimit: number; servicelimit: number; availablelimit: number; };
  ...
  total_assured_service_amount: { totalcarelimit: number; servicelimit: number; availablelimit: number; };
  ...
  checkInnerPermission() {
    ...
    if (checkData) {
      if (checkData.isChildKey == true) {
        ...
        if (this.userType === 'Dental') {
          if (checkSubmenu.hasOwnProperty("dental-appointment")) {
            this.innerMenuPremission = checkSubmenu['dental-appointment'].inner_menu;
        ...
        if (this.userType === 'Optical') {
          if (checkSubmenu.hasOwnProperty("optical-appointmnent")) {
            this.innerMenuPremission = checkSubmenu['optical-appointmnent'].inner_menu;
            console.log(` exists in the object.`);
          } else {
            console.log(`does not exist in the object.`);
          }
        ...
        if (this.userType === 'Laboratory-Imaging') {
          if (checkSubmenu.hasOwnProperty("imaging-appointment")) {
            this.innerMenuPremission = checkSubmenu['imaging-appointment'].inner_menu;
            console.log(` exists in the object.`);
          } else {
            console.log(`does not exist in the object.`);
          }
        ...
        if (this.userType === 'Paramedical-Professions') {
          if (checkSubmenu.hasOwnProperty("paramedical-professions-appointment")) {
            this.innerMenuPremission = checkSubmenu['paramedical-professions-appointment'].inner_menu;
            console.log(` exists in the object.`);
          } else {
            console.log(`does not exist in the object.`);
          }
  ...
  handleAfterConfirmation() {
    ...
    // }
    console.log(this.approvedAmountForAllMedicine, "this.approvedAmountForAllMedicine");
  ...
  handleSend(status) {
    ...
    this.service.addClaimForInsuranceStaff(reqData).subscribe((res) => {
      ...
      if (response.status) {
        ...
        this.router.navigate([`/insurance/appointment-claim/list/${this.userType}`]);
      }
    });
  }
  ...
  approveOrRejectClaim(status: any) {
    ...
    this.service.approveOrRejectClaim(reqData).subscribe((res) => {
      ...
      if (response.status) {
        this.toastr.success(response.message);
        this.closePopup();
        this.router.navigate(["/insurance/appointment-claim"]);
      }
    });
  }
  ...
  async extractPreviousClaims(previousClaims) {
    ...
    await previousClaims.forEach((element) => {
      ...
      if (element.claimType === "Laboratory-Imaging-appointment" && element.claimId != this.claimId) {
  ...
  async handleViewPreviousClaim(claimId, claimtype) {
    ...
    let url = `insurance/appointment-claim/details/${claimId}`;
  ...
  routeToClaimList() {
    this.router.navigate(["/appointment-claim/medicines"]);
  }
  ...
  findCategoryGraph(data: any, subscriberId: any, plan_validity: any) {
    ...
    console.log(reqData, "reqData123");
    console.log(data, "reqData123");
  ...
  doughnutChart1(totalcarelimit: any, subscriberId: any, plan_validity: any, type: any) {
    ...
    // let total_service_limit=0;
    // servicelimit.forEach((item)=>{
    //   total_service_limit += item.service_limit;
    // })
  ...
  doughnutChart2(totallimit: any, subscriberId: any, plan_validity: any) {
    ...
    // let total_service_limit=0;
    // servicelimit.forEach((item)=>{
    //   total_service_limit += item.family_service_limit;
    // })
    ...
    this.service.categoryclaimGraph(reqData).subscribe({
      next: async (res) => {
        ...
        console.log(this.family_total_insured_amount, "sggshgjghh");

        if (this.family_total_insured_amount?.availablelimit < 0) {
          this.doughnutChartDatasets = [
            {
              data: [this.family_total_insured_amount.servicelimit, 0],
              label: "Reputation",
              backgroundColor: ["#FFD57A", "#4F90F2"],
              hoverBackgroundColor: ["#FFD57A", "#4F90F2"],
              hoverBorderColor: ["#FFD57A", "#4F90F2"],
  ...
  claimTypeFilter(event) {
    ...
    if (event.value.subscription_for === 'Primary') {
      // console.log("123",this.primaryplanServiceData)
      // console.log("456",this.dataSourceService)
