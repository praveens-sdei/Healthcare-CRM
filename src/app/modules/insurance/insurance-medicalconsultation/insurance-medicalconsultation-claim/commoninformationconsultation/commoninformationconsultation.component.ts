...
@Component({
  selector: 'app-commoninformationconsultation',
  templateUrl: './commoninformationconsultation.component.html',
  styleUrls: ['./commoninformationconsultation.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CommoninformationconsultationComponent implements OnInit {
    ...
    this.pharmacyService.medicineClaimDetails(this.selectclaimid).subscribe({
      next: async (res) => {
        ...
        if (result.status) {
          ...
          this.deliverTitleselected = claimData?.deliverCenterInfo?.deliverTitleId;
          ...
          console.log("fifth", this.prescriberSpecialitydefault, this.fourthFormGroup, this.prescriberCenter1);
          this.doctorList1 = claimData?.prescriberCenterInfo?.doctorList;
          this.prescriberCenter1 = claimData?.prescriberCenterInfo?.prescriberCenter;
  ...
  private getInsuranceAcceptedListDoctor() {
    ...
    if (this.loggedRole == "INSURANCE_ADMIN") {
      ...
      this.insuraneList = this.adminDataId
      ...
      this.getInsuranceId(this.insuraneList)
    }
    console.log(this.loggedId, "check idssssssss");
    /* old code */
    console.log(this.adminData, "check data admin");
    let param;
    if (this.loggedRole == "patient") {
      param = {
  ...
  onPrescriberCenterChange(selectedLocation: any) {
    ...
    if (filterObject[0].for_location_info?.locationFor == "clinic") {
      if (filterObject[0]?.type == 'Dental' || filterObject[0]?.type == 'Laboratory-Imaging' || filterObject[0]?.type == 'Optical' || filterObject[0]?.type == 'Paramedical-Professions') {
        this.getFourPortalAsperlocation();
      } else {
        this.getDoctorAsperlocation()
      }
    } else {
      this.getDoctorList();
    }
  ...
  getDoctorAsperlocation() {
    let reqData = {
      clinic_id: localStorage.getItem('selected-location-id')
    };
    console.log(reqData, "RESPONSE==========>12");

    this.hospitalservice.getDoctorasPerlocList(reqData).subscribe((res) => {
      let response = this.coreService.decryptObjectData({ data: res });
      ...
      if (this.editClaimData != "") {
        ...
        this.fourthFormGroup.patchValue({
          prescriberSelectedId: this.editClaimData?.prescriberCenterInfo?.prescriberCenter,
          prescriberTitle: this.editClaimData?.prescriberCenterInfo?.prescriberTitle,
          prescriberSpecialty:
            this.editClaimData?.prescriberCenterInfo?.prescriberSpecialty,
  ...
  getFourPortalAsperlocation() {
    let reqData = {
      clinic_id: this.locationId
    };
  ...
  onPrescriberDoctorList(selectedValue: any) {
    ...
    if (selectedValue && selectedValue !== 'undefined-undefined-undefined') {
    ...
    if (selectedDoctor) {
      ...
      this.selectedPortalType = localStorage.getItem('selected-four-portal-type')
      if (this.selectedPortalType === 'Hospital') {
        localStorage.setItem("selected-doctor-id", doctorId);
        localStorage.setItem("selected-four-portal-type", selectedDoctor?.for_portal_user?.type);
      } else {
        localStorage.setItem("selected-doctor-id", doctorId);
      }
      ...
      this._pharmacyService
        ...
        .subscribe((res) => {
          ...
          if (response.status) {
            ...
            console.log(response?.body?.prescriberCenterDetails, "  response?.body?.prescriberCenterDetails?.prescriberCenter,");

            this.fourthFormGroup.patchValue({
              prescriberSelectedId: response?.body?.prescriberCenterDetails?.prescriberCenter,
              prescriberFName:
            ...
            this.prescriberSpecialitydefault =
              response?.body?.prescriberCenterDetails?.prescriberSpeciality;
            console.log("fourth", this.prescriberSpecialitydefault, this.fourthFormGroup, this.prescriberCenter1);

            this.doctorList1 =
              response?.body?.prescriberCenterDetails?.doctorList;
            this.prescriberSelectedId = response?.body?.prescriberCenterDetails?.prescriberCenter;
          } else {
            this.ePrescNumber = "";
            this.coreService.showError(response.message, "");
          }
  ...
  getAllHospitalList() {
    this.patientService.getallClinicHospital(this.searchText).subscribe((res) => {
      let response = this.coreService.decryptObjectData({ data: res });
      ...
      this.locationList12.map((curentval, index) => {
        this.locationListAllData.push({
          ...
          label:
            curentval?.for_location_info?.hospital_name
        });
      });
