...
@Component({
  selector: 'app-esignature-consultation',
  templateUrl: './esignature-consultation.component.html',
  styleUrls: ['./esignature-consultation.component.scss']
})
export class EsignatureConsultationComponent implements OnInit {
 // UAB1 table
 ...
 ngAfterViewInit() {
   ...
   this.coreService.getLocalStorage("adminData").company_name ?  this.coreService.getLocalStorage("adminData").company_name :  this.coreService.getLocalStorage("adminData").adminCompanyName;
 ...
 ngOnChanges() {
   console.log("ng on chnage call aesignature", this.mstepperr);
 }
 ...
 private getClaimDetails() {
   console.log(this.claimId, "check claim id 0000");
   ...
   this.pharmacyPlanService.medicineClaimDetails(this.claimId).subscribe({
     next: async (res) => {
       ...
       if (claimData?.previewtemplateUrl) {
         this.pdfSrc = claimData?.previewtemplateUrl;
       }
       console.log("e signature getClaimDetails", claimData);
       ...
       console.log(this.claimMedicine, "check claimMedicineId");
       ...
       console.log(claimData?.eSignature?.signature, "claimData?.eSignature?.signature");
       ...
       if (this.signatureURL != "" && this.signatureURL != undefined) {
 ...
 //     pharm_signature: this.pharm_signature,
 ...
 //   console.log(reqData, "reqData check data");

 //   this.pharmacyPlanService.finalSubmitClaimandPdf(reqData).subscribe(
 //     async (res) => {
 ...
 submitClaim() {
   ...
   //       ) as HTMLElement;
   //       element1.click();
   //       var file = new File([pdfdata], this.claimId + ".pdf");
   //       await this.uploadPreviewTemplate(file).then((res: any) => {
   ...
   console.clear()
   console.log("FINAL SUBMIT REQUEST==>", data)
   // return;

   this.pharmacyPlanService.finalSubmitClaimDoctor(data).subscribe({
     next: async (res) => {
       ...
       if (result.status) {
         ...
         this.closePopUp();
         this.router.navigate(["/insurance/insurance-medical-claim"]);
       } else {
