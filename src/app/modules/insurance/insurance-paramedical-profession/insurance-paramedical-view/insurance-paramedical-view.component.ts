...
@Component({
  selector: 'app-insurance-paramedical-view',
  templateUrl: './insurance-paramedical-view.component.html',
  styleUrls: ['./insurance-paramedical-view.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class InsuranceParamedicalViewComponent implements OnInit {
  ...
  checkInnerPermission(){
    ...
    if(checkData){
      if(checkData.isChildKey == true){
        ...
        if (checkSubmenu.hasOwnProperty("paramedical-professions-claim")) {
          this.innerMenuPremission = checkSubmenu['paramedical-professions-claim'].inner_menu;
  ...
  getMedicineClaimList(sort: any = '') {
    let reqData = {
      ...
      claimType: "Paramedical-Professions-order",
      // requestType: "medical-products",
      sort: sort
    };
    console.log("Req data----->", reqData);
    this.service.medicineConsultationListInsurance(reqData).subscribe(
      (res) => {
  ...
  countClaims() {
    let reqData = {
      ...
      role: this.staffRole,
      insuranceStaffId: this.staffId,
      claimType: "Paramedical-Professions",
      requestType: "medical-products"
    };
    console.log("COUNT REQUEST======>", reqData);
    this.service.medicineClaimCountByStatusInsuranceDoctor(reqData).subscribe((res) => {
      let response = this.coreService.decryptObjectData({ data: res });
  ...
  async handleViewDetails(claimId) {
    console.log(claimId);
    this.router.navigate(["/insurance/paramedical-professions/details"], {
      queryParams: {
        claimId: claimId,
      },
    });
  ...
  exportLab() {
    ...
    this.service.medicineConsultationListInsurance(reqData).subscribe((res) => {
      let response = this.coreService.decryptObjectData({ data: res });
