...
@Component({
  selector: 'app-managepermission-optical',
  templateUrl: './managepermission-optical.component.html',
  styleUrls: ['./managepermission-optical.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ManagepermissionOpticalComponent implements OnInit {
    ...
    if (this.staffID) {
      ...
      this.doctorService.asignMenuSubmit(data).subscribe(
        (res: any) => {
          ...
          this._route.navigate(["/hospital/manageoptical"]);
        },
        (err) => {
          this.loader.stop();
          console.log(err);
    ...
    this.doctorService.getAllMenus(params).subscribe(
      (res: any) => {
        ...
        for (const data of decryptedData.body) {
          if (data._id != "6548dd3786a58c0e47db378b" && data._id != "6548dd3786a58c0e47db378c" && data._id != "6548dd3786a58c0e47db379e") {
            if (data.parent_id === "0") {
              const menuobj = {};
              menuobj[data._id] = [];
