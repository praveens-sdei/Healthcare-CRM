...
export class BasicinfoComponent implements OnInit {
  ...
  getDoctorDetails(fromParent: any) {
    ...
    console.log("GET DOCTOR DETAILS BASIC INFOO=======>", response);   
  ...
  getDepartments() {
    ...
    this.service.getAllDepartment(reqData).subscribe((res) => {
      ...
      this.basicInfo.patchValue({
       department: this.profileDetails?.department?._id
      })
    });
  }
  ...
  getServices(departmenttId: any, type = '') {
    if (departmenttId != undefined) {
      ...
      this.service.getAllServices(reqData).subscribe((res) => {
        ...
        if (type == 'service') {
        ...
        } else if (this.profileDetails?.department?._id != departmenttId) {
          this.basicInfo.patchValue({
            service: ''
          })
        } else {
          ...
          this.basicInfo.patchValue({
            service: this.profileDetails?.services?._id
          })
        }
      });
    }
  ...
  getUnits(serviceId: any, type = '') {
    if (serviceId != undefined) {
      ...
      this.service.getAllUnits(reqData).subscribe((res) => {
        ...
        console.log("this.profileDetails?.unit", this.profileDetails?.unit?._id, this.locationData?.village);

        if (type == 'unit') {
          this.patchValues(this.profileDetails)
        } else if (this.profileDetails?.services?._id != serviceId) {
          this.basicInfo.patchValue({
            unit: ''
          })
        } else {
          this.basicInfo.patchValue({
            unit: this.profileDetails?.unit?._id
          })
        }
      });
  ...
  getExperties() {
    this.service.getAllExperties(this.hospitalId).subscribe((res) => {
      ...
      this.basicInfo.patchValue({
        expertise: this.profileDetails?.expertise?._id,
       });
      console.log("expertiesListt==========>",  this.profileDetails);    
    });
  }
  ...
  getServiceDepartmentUnit(id: any, type: any) {
   ...
   if(type === "selected"){
       this.basicInfo.patchValue({
          departmentt: this.profileDetails?.department?._id
        });
   }
      // if (type === "unit") {
      //   this.basicInfo.patchValue({
