...
@Component({
  selector: 'app-managepermission-paramedical',
  templateUrl: './managepermission-paramedical.component.html',
  styleUrls: ['./managepermission-paramedical.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ManagepermissionParamedicalComponent implements OnInit {
  ...
  assignSubmit() {
    ...
    this.selectedMenuItem.forEach((ele1) => {
      ...
      if (ele1[objData].length > 0 && ele1.isMenuSelected) {
        ...
        subKey["module_type"] = "Paramedical-Professions";
...
// return
    if (this.staffID) {
      ...
      this.doctorService.asignMenuSubmit(data).subscribe(
        (res: any) => {
          ...
          this._route.navigate(["/hospital/manage-paramedical-professions"]);
        },
        (err) => {
          console.log(err);
        }
  ...
  async getLoginMenus() {
    ...
    const params = {
      module_name: "superadmin",
      module_type: "Paramedical-Professions",
    };
    this.doctorService.getAllMenus(params).subscribe(
      (res: any) => {
        ...
        for (const data of decryptedData.body) {
          if (data._id != "65424067f26ccee90223d854" && data._id != "65424067f26ccee90223d855" && data._id != "65424067f26ccee90223d867") {
            if (data.parent_id === "0") {
              const menuobj = {};
              menuobj[data._id] = [];
