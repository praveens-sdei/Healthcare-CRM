...
@Component({
  selector: 'app-documentmanage-paramedical',
  templateUrl: './documentmanage-paramedical.component.html',
  styleUrls: ['./documentmanage-paramedical.component.scss']
})
export class DocumentmanageParamedicalComponent implements OnInit {
  ...
  getDoctorDetails(fromParent: any) {
    ...
    // this.docDetails =
    //       response?.data?.result[0]?.in_document_management?.document_details;
    this.docDetails =response?.data?.documentManagementList[0]?.document_details;
        this.patchValues(this.docDetails);
  }
