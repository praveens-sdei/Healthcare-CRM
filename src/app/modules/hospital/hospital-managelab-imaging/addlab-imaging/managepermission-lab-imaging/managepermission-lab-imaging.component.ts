...
@Component({
  selector: 'app-managepermission-lab-imaging',
  templateUrl: './managepermission-lab-imaging.component.html',
  styleUrls: ['./managepermission-lab-imaging.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ManagepermissionLabImagingComponent implements OnInit {
  ...
  assignSubmit() {
    ...
    this.selectedMenuItem.forEach((ele1) => {
      ...
      if (ele1[objData].length > 0 && ele1.isMenuSelected) {
        ...
        subKey["module_type"] = "Laboratory-Imaging";
...
// return
    if (this.staffID) {
      ...
      this.fourportalservice.asignMenuSubmit(data).subscribe(
        (res: any) => {
          ...
          this._route.navigate(["/hospital/managelaboratoryimaging"]);
        },
        (err) => {
          this.loader.stop();
          console.log(err);
  ...
  async getLoginMenus() {
    ...
    const params = {
      module_name: "superadmin",
      module_type: "Laboratory-Imaging",
    };
    this.doctorService.getAllMenus(params).subscribe(
      (res: any) => {
        ...
        for (const data of decryptedData.body) {
          if (data._id != "6548d3d886a58c0e47db3772" && data._id != "6548d3d886a58c0e47db3773" && data._id != "6548d3d886a58c0e47db3785") {
            if (data.parent_id === "0") {
              const menuobj = {};
              menuobj[data._id] = [];
