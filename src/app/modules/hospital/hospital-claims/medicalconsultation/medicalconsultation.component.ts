...
@Component({
  selector: 'app-medicalconsultation',
  templateUrl: './medicalconsultation.component.html',
  styleUrls: ['./medicalconsultation.component.scss'],
...
export class MedicalconsultationComponent implements OnInit {
  ...
  getStatusCount() {
    ...
    this.pharmacyPlanService.medicineClaimCountByStatusPharmacyDoctor(reqData.pharmacyId, this.selectedinsurance.join(","))
      .subscribe((res) => {
        let response = this.coreService.decryptObjectData({ data: res });
  ...
  async goToNewEdit(claimId) {
    ...
    this.router.navigate(["/hospital/claims/medicalconsultaionclaimsdetails"], {
      queryParams: {
        claimId: claimId,
      },
    });
