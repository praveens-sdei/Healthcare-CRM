import { Component, OnInit } from "@angular/core";
import { ModalDismissReasons, NgbModal } from "@ng-bootstrap/ng-bootstrap";
import { FormArray, FormBuilder, FormControl, FormGroup, Validators } from "@angular/forms";
import { CoreService } from "src/app/shared/core.service";
import { ToastrService } from "ngx-toastr";
import { HospitalService } from "../../hospital.service";
import * as XLSX from "xlsx";
import { NgxUiLoaderService } from "ngx-ui-loader";


// Expertise table data
export interface ExpertisePeriodicElement {
  expertise: string;
  addedby: string;
}
const EXPERTISE_ELEMENT_DATA: ExpertisePeriodicElement[] = [
  { expertise: "Lorem Ipsum", addedby: "Hospital" },
  { expertise: "Lorem Ipsum", addedby: "Hospital" },
  { expertise: "Lorem Ipsum", addedby: "Hospital" },
  { expertise: "Lorem Ipsum", addedby: "Hospital" },
];

@Component({
  selector: "app-expertise",
  templateUrl: "./expertise.component.html",
  styleUrls: ["./expertise.component.scss"],
})
export class ExpertiseComponent implements OnInit {
  // Expertise table data
  expertisedisplayedColumns: string[] = [
    "expertise",
    "addedby",
    "status",
    "action",
  ];
  expertisedataSource = EXPERTISE_ELEMENT_DATA;
  editExpertiseForm!: FormGroup;
  expertiseForm!: FormGroup;
  expertiseId: any;
  page: any = 1;
  pageSize: number = 5;
  totalLength: number = 0;
  searchText: any = "";
  isSubmitted: boolean = false;
  userId: any = "";
  selectedFiles: File;

  sortColumn: string = 'expertise';
  sortOrder: 'asc' | 'desc' = 'asc';
  sortIconClass: string = 'arrow_upward';
  userRole: any;
  userPermission: any;
  innerMenuPremission: any = [];
  constructor(
    private modalService: NgbModal,
    private fb: FormBuilder,
    private service: HospitalService,
    private _coreService: CoreService,
    private toastr: ToastrService,
    private loader: NgxUiLoaderService
  ) {
    this.expertiseForm = this.fb.group({
      expertisess: this.fb.array([]),
    });

    let loginData = JSON.parse(localStorage.getItem("loginData"));
    let admindata = JSON.parse(localStorage.getItem("adminData"));
    this.userRole = loginData?.role;
    this.userPermission = loginData?.permissions;

    if (this.userRole === "HOSPITAL_STAFF") {
      this.userId = admindata?.in_hospital;

    } else {
      this.userId = loginData?._id;
    }

    this.editExpertiseForm = this.fb.group({
      expertiseId: ["", [Validators.required]],
      expertise: ["", [Validators.required]],
      active_status: ["", [Validators.required]],
    });
  }

  onSortData(column: any) {
    this.sortColumn = column;
    this.sortOrder = this.sortOrder === 'asc' ? 'desc' : 'asc';
    this.sortIconClass = this.sortOrder === 'asc' ? 'arrow_upward' : 'arrow_downward';
    this.getAllExpertise(`${column}:${this.sortOrder}`);
  }

  ngOnInit(): void {
    this.addnewExpertise();
    this.getAllExpertise(`${this.sortColumn}:${this.sortOrder}`);
    setTimeout(() => {
      this.checkInnerPermission();
    }, 300);
  }

  findObjectByKey(array, key, value) {
    return array.find(obj => obj[key] === value);
  }

  checkInnerPermission() {
    let menuID = sessionStorage.getItem("currentPageMenuID");
    let checkData = this.findObjectByKey(this.userPermission, "parent_id", menuID)
    if (checkData) {
      if (checkData.isChildKey == true) {
        var checkSubmenu = checkData.submenu;
        if (checkSubmenu.hasOwnProperty("department")) {
          this.innerMenuPremission = checkSubmenu['department'].inner_menu;
          console.log(`exist in the object.`);
        } else {
          console.log(`does not exist in the object.`);
        }
      } else {
        var checkSubmenu = checkData.submenu;
        let innerMenu = [];
        for (let key in checkSubmenu) {
          innerMenu.push({ name: checkSubmenu[key].name, slug: key, status: true });
        }
        this.innerMenuPremission = innerMenu;
        console.log("innerMenuPremission________", this.innerMenuPremission);

      }
    }
  }


  giveInnerPermission(value) {
    if (this.userRole === "HOSPITAL_STAFF") {
      const checkRequest = this.innerMenuPremission.find(request => request.slug === value);
      return checkRequest ? checkRequest.status : false;
    } else {
      return true;

    }
  }
  expertiseExcleForm: FormGroup = new FormGroup({
    vaccination_csv: new FormControl("", [Validators.required]),
  });
  //------------apiCall--------------->
  getAllExpertise(sort: any = '') {
    let reqData = {
      added_by: this.userId,
      limit: this.pageSize,
      page: this.page,
      searchText: this.searchText,
      sort: sort
    };
    this.service.getExpertiseApi(reqData).subscribe((res) => {
      let encryptedData = { data: res };
      let response = this._coreService.decryptObjectData(encryptedData);
      this.totalLength = response?.body?.totalCount;
      this.expertisedataSource = response?.body?.data;
    });
  }
  addExpertise() {
    this.isSubmitted = true;
    if (this.expertiseForm.invalid) {
      this._coreService.showError("","Please Fill Required fields")
      return;
    }
    let reqData = {
      expertiseArray: this.expertiseForm.value.expertisess,
      added_by: this.userId,
    };
    this.loader.start();
    this.isSubmitted = false;
    this.service.addExpertiseApi(reqData).subscribe((res: any) => {
      let encryptedData = { data: res };
      let response = this._coreService.decryptObjectData(encryptedData);
      if (response.status) {
        this.loader.stop();
        this.toastr.success(response.message);
        this.getAllExpertise();
        this.closePopup();
      } else {
        this.loader.stop();
        this.toastr.error(response.message);
      }
    });
  }
  updateExpertise() {
    this.isSubmitted = true;
    if (this.editExpertiseForm.invalid) {
      this._coreService.showError("","Please Fill Required fields")
      return;
    }
    let reqData ={
      added_by: this.userId,
      ...this.editExpertiseForm.value,
    }
    this.loader.start();
    this.service
      .updateExpertiseApi(reqData)
      .subscribe((res: any) => {
        let encryptedData = { data: res };
        let response = this._coreService.decryptObjectData(encryptedData);
        if (response.status) {
          this.loader.stop();
          this.getAllExpertise();
          this.toastr.success(response.message);
          this.closePopup();
        } else {
          this.loader.stop();
          this.toastr.error(response.message);
        }
      });
  }
  deleteExpertise() {
    this.loader.start();
    let reqData = {
      expertiseId: this.expertiseId,
      action_name: "delete",
      action_value: true,
    };
    this.service.deleteExpertiseApi(reqData).subscribe((res: any) => {
      let encryptedData = { data: res };
      let response = this._coreService.decryptObjectData(encryptedData);
      if (response.status) {
        this.loader.stop();
        this.getAllExpertise();
        this.toastr.success(response.message);
        this.closePopup();
      } else {
        this.loader.stop();
        this.toastr.error(response.message);
      }
    });
  }

  handletoggleChange(event: any, data: any) {
    this.editExpertiseForm.patchValue({
      expertiseId: data?._id,
      expertise: data?.expertise,
      active_status: event.checked,
    });
    this.updateExpertise();
  }
  handleSearchCategory(event: any) {
    this.searchText = event.target.value;
    this.getAllExpertise();
  }
  handlePageEvent(data: any) {
    this.page = data.pageIndex + 1;
    this.pageSize = data.pageSize;
    this.getAllExpertise();
  }
  closePopup() {
    this.modalService.dismissAll("close");
    this.expertiseForm.reset();
  }

  //-------Form Array Handling--------------->
  newexpertiseForm(): FormGroup {
    return this.fb.group({
      expertise: ["", [Validators.required]],
      active_status: [true],
    });
  }

  get expertisess(): FormArray {
    return this.expertiseForm.get("expertisess") as FormArray;
  }

  addnewExpertise() {
    this.expertisess.push(this.newexpertiseForm());
    this.isSubmitted
  }

  removeExpertise(i: number) {
    this.expertisess.removeAt(i);
  }
  //  Add Expertise service modal
  openVerticallyCenteredAddExpertiseServicecontent(
    addExpertiseServicecontent: any
  ) {
    console.log("Add Call");

    this.modalService.open(addExpertiseServicecontent, {
      centered: true,
      size: "md",
      windowClass: "add_Expertiseservice",
    });
  }
  //  Edit Expertise service modal
  openVerticallyCenteredEditExpertiseService(
    editExpertiseServicecontent: any,
    data: any
  ) {
    this.editExpertiseForm.patchValue({
      expertiseId: data._id,
      expertise: data.expertise,
      active_status: data.active_status,
    });
    this.modalService.open(editExpertiseServicecontent, {
      centered: true,
      size: "md",
      windowClass: "edit_expertise_service",
    });
  }

  //delete popup
  openVerticallyCenteredsecond(deletePopup: any, expertiseId: any) {
    this.expertiseId = expertiseId;
    this.modalService.open(deletePopup, { centered: true, size: "sm" });
  }
  private getDismissReason(reason: any): string {
    if (reason === ModalDismissReasons.ESC) {
      return "by pressing ESC";
    } else if (reason === ModalDismissReasons.BACKDROP_CLICK) {
      return "by clicking on a backdrop";
    } else {
      return `with: ${reason}`;
    }
  }


  openVerticallyCenteredimport(imporMedicine: any) {
    this.modalService.open(imporMedicine, {
      centered: true,
      size: "lg",
      windowClass: "master_modal Import",
    });
  }

  downLoadExcel() {
    const link = document.createElement("a");
    link.setAttribute("target", "_blank");
    link.setAttribute("href", "assets/doc/expertise_hospital.xlsx");
    link.setAttribute("download", `expertise_hospital.xlsx`);
    document.body.appendChild(link);
    link.click();
    link.remove();
  }
  fileChange(event) {
    let fileList: FileList = event.target.files;
    if (fileList.length > 0) {
      let file: File = fileList[0];
      this.selectedFiles = file;
    }
  }
  excleSubmit() {
    this.loader.start();
    this.isSubmitted = true;
    if (this.expertiseExcleForm.invalid) {
      return;
    }
    const formData = new FormData();
    formData.append("user_id", this.userId);
    formData.append("file", this.selectedFiles);
    // uploadExcelMedicine
    this.service.uploadExcelforExpertise(formData).subscribe(
      (res: any) => {
        let encryptedData = { data: res };
        let response = this._coreService.decryptObjectData(encryptedData);

        if (response.status === true) {
          this.loader.stop();
          this.toastr.success(response.message);
          this.closePopup();
          this.getAllExpertise();
          this._coreService.setCategoryForService(1);
        } else {
          this.loader.stop();
          this.toastr.error(response.message);
        }
      },
      (error: any) => {
        this.loader.stop();
        let encryptedData = { data: error.error };
        let response = this._coreService.decryptObjectData(encryptedData);
        if (!response.status) {
          this.toastr.error(response.message);
        }
      }
    );
  }


  exportVaccination() {
    this.loader.start();
    /* generate worksheet */
    var data: any = [];
    this.pageSize = 0;
    this.service
      .expertiseListforexport(this.page, this.pageSize, this.searchText, this.userId)
      .subscribe((res) => {
        let result = this._coreService.decryptObjectData({ data: res });

        if (result.status == true) {
          this.loader.stop();
          var array = ["expertise_name", "added_by"];

          data = result.data.array;

          data.unshift(array);
          console.log("data", data);

          var fileName = "SheetJS.xlsx";

          const ws: XLSX.WorkSheet = XLSX.utils.aoa_to_sheet(data);
          /* generate workbook and add the worksheet */
          const wb: XLSX.WorkBook = XLSX.utils.book_new();
          XLSX.utils.book_append_sheet(wb, ws, "Sheet1");
          /* save to file */
          XLSX.writeFile(wb, fileName);
        }
      });
  }
}
