...
@Component({
  selector: "app-basicinfo-dental",
  templateUrl: "./basicinfo-dental.component.html",
  styleUrls: ["./basicinfo-dental.component.scss"],
})
export class BasicinfoDentalComponent implements OnInit {
  ...
  getDoctorDetails(fromParent: any) {
    ...
    this.license_picture_ = this.profileDetails?.license_details?.image;
    // console.log("license_picture_", this.license_picture_);
  ...
  patchValues(data: any) {
    ...
    console.log("this.selectedSpeciality>>>", this.selectedcountrycodedb);
    // console.log("this.selectedcountrycodedb>>>", this.selectedcountrycodedb);
    this.getCountryCode();
  }
  ...
  onFileSelected(event, type: "profile" | "license") {
    ...
    formData.append("portalType", "Dental");
  ...
  getExperties() {
    this.service.getAllExperties(this.hospitalId).subscribe((res) => {
      ...
      this.basicInfo.patchValue({
        expertise: this.profileDetails?.expertise[0]._id,
      });
