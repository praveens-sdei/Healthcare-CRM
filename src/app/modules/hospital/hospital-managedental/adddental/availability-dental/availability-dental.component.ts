...
@Component({
  selector: 'app-availability-dental',
  templateUrl: './availability-dental.component.html',
  styleUrls: ['./availability-dental.component.scss']
})
export class AvailabilityDentalComponent implements OnInit {
  ...
  patchValues(element: any) {
    ...
    if (element.length != 0) {
      ...
      if (Object.keys(onlineMode).length != 0) {
        ...
        console.log("this.availabilityFormOnline>>>>>>", onlineMode?.slot_interval)
      } else {
        this.addNewWeek("online");
        this.addNewAvailabilty("online");
        this.addNewUnAvailabilty("online");
