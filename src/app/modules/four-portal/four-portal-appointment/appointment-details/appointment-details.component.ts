...
export class AppointmentDetailsComponent implements OnInit {
  ...
  endConsultation() {
    if (
      this.paymentType === "post-payment" &&
      this.subscriberDetails.length > 0 &&
      this.subsciberId
    ) {
      if (!this.appointmentPayment) {
        this.coreService.showInfo("Please Paid the Co-pay Amount", "");
      } else {
  ...
  private async updatePaymentDetails() {
    console.log("this.paymentDetails______________",this.paymentDetails);
    
    if (this.paymentDetails != "" && this.paymentDetails != null) {

      for await (const iterator of this.paymentDetails) {

        console.log("iterator______________", iterator);
    ...
    this.fourPortalService.fourPortal_paymentReceived(reqData).subscribe(
      (res) => {
        ...
        if (response.status) {
          ...
          this.route.navigate([`portals/appointment/${this.userType}`]);
        }
      },
      (err) => {
        let errResponse = this.coreService.decryptObjectData({
  ...
  acceptOrRejectAppointment(status: any, reason: any) {
    let reqData = {
      ...
      cancelReason: reason,
      status: status,
      cancelledOrAcceptedBy: this.portalId,
      loginId: this.portalId,
  ...
  assignHeathCareProvider() {
    ...
    this.fourPortalService
      .fourPortal_assignedStaff(reqData)
      .subscribe((res) => {
        let response = this.coreService.decryptObjectData({ data: res });
        if (response.status) {
          this.toastr.success(response.message);
  ...
  fourportal_Details() {
    let param = { portal_user_id: this.portalId, type: this.userType };
  ...
  handleRescheduleAppointment(isNextAvailable: any) {
    let reqData = {
      ...
      rescheduled_by_id: this.portalId,
    };
    ...
    let data = {
      ...
      this.pharmacyPlanService.getInsuranceAcceptedListForFourPortal(data).subscribe({
        next: (res) => {
          ...
          let encryptedData = { data: res };
          let result = this.coreService.decryptObjectData(encryptedData);
console.log("insuranceId____________",this.insuranceId);

            if (result.status === true) {
              const matchingRecord = result.body.find(ele => ele.for_portal_user === this.insuranceId);            
              if (matchingRecord) {                
                this.getPortalTypeAndInsuranceId(matchingRecord.for_portal_user)
            } else {
              this.serviceValue = 0;
              this.copayment = this.consultationFee;
              this.insurancePay = 0;
              if(this.appointmentDetails.paymentdetails === null){
