...
@Component({
  selector: 'app-commoninformation-appointment',
  templateUrl: './commoninformation-appointment.component.html',
  styleUrls: ['./commoninformation-appointment.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class CommoninformationAppointmentComponent implements OnInit {
  ...
  constructor(
    ...
    console.log(this.adminData_id, "local id admin");
    this.loggedType = this.coreService.getLocalStorage("loginData").type;
    let loggedIdLocal = this.coreService.getLocalStorage("loginData")._id;
    console.log(loggedIdLocal, "local id");
    this.loggedIdProfile = this.coreService.getLocalStorage("loginData")._id;
    ...
    this.pharmacyService.medicineClaimDetails(this.selectclaimid).subscribe({
      next: async (res) => {
        ...
        if (result.status) {
          ...
          // this.doctorListselected = claimData?.deliverCenterInfo?.deliverTitleId;

          this.selectedDeliveryStaff =
            claimData?.deliverCenterInfo?.deliverFirstName;

          this.prescriberCenter = claimData?.prescriberCenterInfo?.prescriberCenter,
            this.deliverTitleselected = claimData?.prescriberCenterInfo?.prescriberTitle,
            this.doctorListselected = claimData?.prescriberCenterInfo?.doctorList,
            // this.prescriberMName = claimData?.prescriberCenterInfo?.prescriberMiddleName,
            // this.prescriberTitle = claimData?.prescriberCenterInfo?.prescriberTitle,
  ...
  getPatientDetails(patient_id: string) {
    const insuranceNo = ''
    const subscriber_id = this.orderDetails.appointmentDetails.subscriber_id
  ...
  private getAllInsurance() {
    ...
    this.pharmacyService.getInsuranceAcceptedListForFourPortal(param).subscribe({
      ...
      next: async (res) => {
        ...
        console.log(this.insuraneList, "insurancelist");
      },
      error: (err: ErrorEvent) => {
        console.log(err.message);
      },
    ...
    this.pharmacyService.checkInsuranceStaff(finalReqData).subscribe({
      next: async (res) => {
        ...
        if (result.data) {
          ...
          this.ngxService.start();
          formVal.pharmacyId = "";
          formVal.createdById = this.loggedIdProfile;
          formVal.claimId = "";
          formVal.created_by = this.loggedType;
  ...
  saveStepTwo(formVal: any, stepper: MatStepper) {
    ...
    let finalReqData = {
      ...
      createdById: this.loggedIdProfile,
  ...
  searchWithEprescriptionNumber(ePrescriptionNumber) {
    ...
    if (ePrescriptionNumber) {
      ...
      this._pharmacyService
        .getAllEprescriptionDetailsForFourPortal(reqData)
        .subscribe((res) => {
          let response = this.coreService.decryptObjectData({ data: res });
          ...
          if (response.body.subscriberId == "") {
            ...
            if (response.status) {
              ...
              localStorage.setItem("selected-location-id", response?.body?.prescriberCenterDetails?.prescriberCenterId,);
              ...
              if (this.loggedRole == 'INDIVIDUAL') {
                this.getDoctorList();
                console.log(this.doctorList, "check doctor list");

              }
              // const filteredDoctors = this.doctorList.filter(doctor => doctorIds.includes(doctor.id));
              ...
              this.prescriberSpecialitydefault = response?.body?.specialityname,
                this.doctorListselected = this.doctorList,
  ...
  getLocations() {
    let param: any;
    if (this.loggedRole == "Laboratory-Imaging") {
      param = {
  ...
  getDoctorList() {
    if (this.loggedRole == "INDIVIDUAL") {
      ...
      let reqData = {
        page: this.page,
        limit: 1000,
        hospital_portal_id: this.selectedLocationid,
        searchText: "",
      ...
      next: (result1: any) => {
        ...
        if (result.status === true) {
          setTimeout(() => {
            ...
            this.subscriberId2 = result?.data?.patientAllDetails?.insuranceDetails?.subscriber_id;
