...
@Component({
  selector: 'app-esignature-appointment',
  templateUrl: './esignature-appointment.component.html',
  styleUrls: ['./esignature-appointment.component.scss']
})
export class EsignatureAppointmentComponent implements OnInit {
    ...
    this.pharmacyPlanService.finalSubmitClaimDoctor(data).subscribe({
      next: async (res) => {
        ...
        if (result.status) {
          ...
          this.closePopUp();
          this.router.navigate([`/portals/dashboard/${this.loggedType}`]);
