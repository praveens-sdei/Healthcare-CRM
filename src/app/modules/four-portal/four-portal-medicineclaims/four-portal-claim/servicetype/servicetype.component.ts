...
export class ServicetypeComponent implements OnInit {
  ...
  async addMedicineForSuperadmin(index) {
    return new Promise((resolve, reject) => {
      ...
      if (this.loggedType == "Paramedical-Professions") {
        // Condition for Paramedical Professions
        medicineArray.push({
          category: "",
          lab_test: this.selectKeyPressValue[index],
          description: "",
        ...
        medicineArray.push({
          category: "",
          imaging: this.selectKeyPressValue[index],
          description: "",
          clinical_consideration: "",
          contributing_factors_to_abnormal: "",
          normal_value: "",
        ...
        medicineArray.push({
          category: "",
          others: this.selectKeyPressValue[index],
          description: "",
          clinical_consideration: "",
          contributing_factors_to_abnormal: "",
          normal_value: "",
        ...
        medicineArray.push({
          eyeglass_name: this.selectKeyPressValue[index],
          status: true,
          is_deleted: false,
          is_new: false
        });
      ...
      if (medicineArray.length > 0) {
        ...
        if (this.loggedType == "Paramedical-Professions") {
          reqData = {
            LabTestArray: medicineArray, // Changed key to LabTestArray
            is_new: false,
            added_by: added_by,
          };
        } else if (this.loggedType == "Laboratory-Imaging") {
          reqData = {
            ImagingTestArray: medicineArray,
            is_new: false,
            added_by: added_by,
          };
        } else if (this.loggedType == "Dental") {
          reqData = {
            OthersTestArray: medicineArray,
            is_new: false,
            added_by: added_by,
          };
        } else if (this.loggedType == "Optical") {
          reqData = {
            eyeglassData: medicineArray,
            is_new: false,
            added_by: added_by,
          };
        }
        ...
        if (this.loggedType == "Laboratory-Imaging") {
          subscription = this._superAdminService.addImagingApi(reqData);
        } else if (this.loggedType == "Paramedical-Professions") {
          subscription = this._superAdminService.labAddTest(reqData);
        } else if (this.loggedType == "Dental") {
          subscription = this._superAdminService.addOthersApi(reqData);
        } else if (this.loggedType == "Optical") {
          subscription = this._superAdminService.addEyeglassessApi(reqData);
        }
        subscription.subscribe(async (res: any) => {
  ...
  getMedicineList(query: any = "", index: any = 0, medicneId: string = "", serviceName: string = '', categoryService: string = '', serviceCode: string = '') {
    ...
    if (this.loggedType == "Paramedical-Professions") {
      this._superAdminService.getLabListData(this.page, this.pageSize, this.selectKeyPressValue[index]).subscribe(
        (res) => {
          let result = this.coreService.decryptObjectData({ data: res });
          console.log(result, "check data all");

          const medicineArray = [];
          this.actualMedicineList[index] = result.data.result
      ...
      let reqData = {
        ...
        searchText: this.selectKeyPressValue[index],
        status: null,
        // sort: sort
      };
    ...
    this.labImaginge.fourPortal_appointment_deatils(reqData).subscribe({
      next: (result1: any) => {
        ...
        if (result.status === true) {
          ...
          var totalCostOfAllMedicine = 0;
          if (result?.data?.testDetails.length > 0) {
            result?.data?.appointmentDetails.forEach(
              (element: any, index: any) => {
                ...
                obj.push({
                  // date_of_service: element?.createdAt,
                  // categoryService: element?.category,
                  // reasonOfConsultation: element?.reasonForAppointment._id,
                  // medicineId: element?.medicine_id,
                  // medicine_name: element?.name,
                  // serviceName: element?.service,
                  // serviceCode: '',
