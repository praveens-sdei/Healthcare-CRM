...
export class CommoninformationComponent implements OnInit {
  ...
  adminData: any;
  loggedRole: any;
  staff_detailsProfile: any;
  staffDetailsRole: any;
  loggedIdProfile: any;
  userType: any;
  prescriberTitledata: any;
  ...
  constructor(
    ...
    console.log(this.loggedIdProfile, "chekc profile");
    let loginData = JSON.parse(localStorage.getItem("loginData"));
    this.userType = loginData?.type
    let loggedIdLocal = this.coreService.getLocalStorage("loginData")._id;

    console.log(loggedIdLocal, "check logged Id");
    this.adminData = this.coreService.getLocalStorage("adminData").creatorId;
    console.log(this.adminData, "check logged Id");

    if (this.loggedRole == "STAFF") {
      this.loggedId = this.adminData;
    }
    ...
    if (this.orderId == undefined && this.appointment == undefined) {
      this.orderId = '';
      this.logginIdOrder = '';
    }
    ...
    if (this.orderId == undefined && this.appointment == undefined) {
      this.appointment = '';
      this.logginIdOrder = '';
      console.log(this.appointment, "check appointmtne");
    }
    ...
    if (this.appointment != "" && this.appointment != undefined) {
      ...
      console.log(this.logginIdOrder, "check appointmtne");
    }
  ...
  ngOnInit() {
    ...
    this.getstaffdetailsProfile(this.loggedIdProfile);
    this.getAllRole();
  ...
  getstaffdetailsProfile(id: any) {
    ...
    this.fourportalService.getStaffDetails(pararm).subscribe({
      next: (res) => {
        ...
        this.staff_detailsProfile = result?.body
        this.staffDetailsRole = this.staff_detailsProfile?.role?._id;
        // this.selectedLanguages = this.staff_details?.in_profile?.language;
        console.log(this.staff_detailsProfile, "staff_detailss____");
        if (this.loggedRole === "STAFF") {
          console.log("check log");

          this.deliverTitleselected = this.staffDetailsRole;
          this.getStaffDetails(true, this.deliverTitleselected)
  ...
  getStaffDetails(event: any, role = '') {
    ...
    this.labImaginge.listCategoryStaff(params).subscribe({
      next: async (res) => {
        let encData = await res;
        let result = this.coreService.decryptContext(encData);
        ...
        let checkstaff;
        if (role == "") {
          checkstaff = this.staffRole.find(
        ...
        if (this.loggedRole === "STAFF" && role != '') {
          this.deliverStaffselected = this.staff_detailsProfile.in_profile.name
        }
      },
    });
    // }
      ...
      next: async (res) => {
        ...
        if (result.status) {
          ...
          if (this.patientDetailList.length > 0) {
            console.log("check AFTER PRESc===> ", this.prescData);
            result.body.map((curentval, index) => {
            ...
            if (this.appointment != '') {
              console.log(this.subscriberId2, "pp333");
  ...
  getPatientDetailsAppointment(patient_id: string) {
    const insuranceNo = ''
    const subscriber_id = this.subscriberId2
    console.log(this.subscriberId2, "this.subscriberId2 ");
  ...
  public getAppointmentDetails(): void {
    ...
    this.labImaginge.fourPortal_appointment_deatils(reqData).subscribe({
      next: (result1: any) => {
        ...
        console.log(result, "check result order");
        this.orderDetails = result?.data
        console.log(this.logginIdOrder, "this.logginIdOrder outside");

        if (result.status === true) {

          this.deliveryCenterselected = this.logginIdOrder;
          this.subscriberId2 = result?.data?.patientAllDetails?.portalUserDetails?._id;
          ...
          this.getPatientDetailsAppointment(result?.data?.patientAllDetails?.portalUserDetails?._id);
        }
      },
      error: (err: ErrorEvent) => {
        this.coreService.showError("", err.message);
