...
@Component({
  selector: 'app-four-portal-mydocument',
  templateUrl: './four-portal-mydocument.component.html',
  styleUrls: ['./four-portal-mydocument.component.scss'],
  encapsulation: ViewEncapsulation.None,

})
export class FourPortalMydocumentComponent implements OnInit {
  ...
  length: number;
  type: any;
  uploadDateControl = new FormControl(new Date());
  constructor(
  ...
  onSubmit() {
    ...
    this.fourPortalService.uploadDocumentProvider(formData).subscribe(
      (res) => {
  ...
  getPortalDocumentList() {
    ...
    this.fourPortalService.getPortaldocumentList(data).subscribe(
      (res) => {
  ...
  toggleStatus1(event, id) {
    ...
    this.fourPortalService.statusUpdateProviderDoc(data).subscribe(
      (res) => {
        let response = this._coreService.decryptObjectData({ data: res });
        if (response.status) {
          this.loader.stop();
  ...
  openVerticallyCenteredsecond1(data1: any) {
    ...
    this.fourPortalService.getProviderDoc(data).subscribe(
      (res) => {
        let response = this._coreService.decryptObjectData({ data: res });
        if (response.status) {
