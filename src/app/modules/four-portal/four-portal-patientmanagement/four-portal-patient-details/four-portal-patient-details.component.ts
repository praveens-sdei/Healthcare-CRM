...
@Component({
  selector: 'app-four-portal-patient-details',
  templateUrl: './four-portal-patient-details.component.html',
  styleUrls: ['./four-portal-patient-details.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class FourPortalPatientDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log("SEARCH PARAMS====>", this.queryParams,this.patientId);
...

{
  ...
  handleRouting(index: any) {
    ...
    this.route.navigate([
      `/portals/patientmanagement/${this.userType}/edit/${this.patient_id}`,
    ]);
  }
  ...
  getPastAppointment() {
    ...
    this.fourPortalService.getPastAppointOfPatient(reqData).subscribe((res) => {
      let response = this.coreService.decryptObjectData({ data: res });
      console.log("PAST APPOINTMENT==>", response);
      this.pastappointmentdataSource = response?.data;
  ...
  getImmunizationOptionsList() {
    this.service.vaccineList().subscribe((res) => {
      let response = this.coreService.decryptObjectData({ data: res });
      this.immunizationOptionsList = response?.data?.result;
    });
  }
  ...
  noPermissionHandler() {
    this.modalService.dismissAll("close");
    this.route.navigate([`/portals/patientmanagement/${this.userType}`]);
  }
  ...
  handleBack() {
    if (Object.keys(this.queryParams).length != 0) {
      // this.route.navigate(["/individual-doctor/appointment/videocall"]);
    } else {
      this.route.navigate([`/portals/patientmanagement/${this.userType}`]);
    }
  }
