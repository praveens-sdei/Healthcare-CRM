...
export class PaymentComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private activeRoute: ActivatedRoute,
    private fourPortalService: FourPortalService, private _coreService: CoreService,
    private fb: FormBuilder, private stripeService: StripeService, private http: HttpClient,private router : Router,
...
console.log("localStorageAdminData>>>>>>>",this.localStorageAdminData)
  ...
  ngOnInit(): void {
    ...
    this.fourPortalService.getSubscriptionPlanDetails(paramData).subscribe({
      next: (res) => {
        let paymentReq = {};
        ...
        paymentReq = {
          ...
          description: 'Portals Plan Subscription',
          ...
          portal_type:this.localStorageUserData.type
  ...
  private createPaymentIntent(reqData: any): Observable<PaymentIntent> {
    console.log("createPaymentIntent=====",reqData)
...
routeTodashboard(){
  this.router.navigate([`/portals/dashboard/${this.localStorageUserData.type}`])
  this.modalService.dismissAll();
}
routeBack(){
  this.router.navigate([`/portals/subscriptionplan/${this.localStorageUserData.type}/plan`])
}
}
