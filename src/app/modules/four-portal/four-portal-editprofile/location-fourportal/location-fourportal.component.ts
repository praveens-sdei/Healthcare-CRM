...
@Component({
  selector: 'app-location-fourportal',
  templateUrl: './location-fourportal.component.html',
  styleUrls: ['./location-fourportal.component.scss']
})
export class LocationFourportalComponent implements OnInit {
  ...
  async saveLocation() {
    ...
    this.fourportalservice.locationDetails(reqData).subscribe(
      (res) => {
        let response = this.coreService.decryptObjectData({ data: res });
        if (response.status) {
          this.loader.stop();
