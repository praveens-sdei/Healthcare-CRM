...
@Component({
  selector: 'app-patient-common-appointment',
  templateUrl: './patient-common-appointment.component.html',
  styleUrls: ['./patient-common-appointment.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class PatientCommonAppointmentComponent implements OnInit {
  ...
  handleservice(target: any, serviceOption: any): void {
    if (target) {
      ...
      if (serviceOption.pre_authorization) {
        ...
        this.serviceMessage = "";
        this.firstFormGroup.patchValue({ requestType: "appointment-claim" });
      }
        ...
        if (result.status) {
          ...
          if (this.patientDetailList.length > 0) {
            ...
            if (this.appointmentId != '') {
              console.log(this.subscriberId2, "pp333", this.doctorListselectedname);

              this.patientSelectedId = await this.subscriberId2;
              this.doctorListselected = await this.doctorListselectedname;
            }
    ...
    this.labimagingdentaloptical.laboratoryList(reqData).subscribe(async (res) => {
      ...
      if (response.status) {
        ...
        for (const curentval of array2) {
          console.log(curentval, "check data hospital123");
  ...
  public saveStepOne(formVal: any, stepper: MatStepper) {
    ...
    formVal.claimType = this.loggedType + "-appointment";
  ...
  //   console.log(selectedLocation, "c4444", this.locationList);
  ...
  onPrescriberCenterChange(selectedLocation: any) {
    console.log(selectedLocation, "c4444");

    let filterObject = this.locationList12.filter(obj => obj?.for_location_info['hospital_name'] === selectedLocation);
    console.log(`filterObject`, filterObject);
    // this.fourthFormGroup.get('prescriberCenter').setValue(selectedLocation.hospital_id);
    this.locationFor = filterObject[0]?.for_location_info?.location;
    this.locationSlected = filterObject[0]?.for_location_info?.hospital_name


    this.locationId = filterObject[0]?.for_location_info?.hospital_id;
    localStorage.setItem("selected-location-id", this.locationId);


    // if (this.loggedRole != "INDIVIDUAL" && this.loggedRole != "HOSPITAL") {
    // if (filterObject[0]?.for_location_info?.locationFor == "clinic") {
    this.getFourPortalAsperlocation(this.locationId);
    // } else {
    //   this.getDoctorList();
    // }
    // }
  ...
  getFourPortalAsperlocation(id: any) {
    let reqData = {
      clinic_id: id,
      type: this.loggedType
    };
    console.log(reqData, "check req");


    this.labImaginge.getFourPortalasPerlocList(reqData).subscribe((res) => {
      let response = this.coreService.decryptObjectData({ data: res });
      ...
      response?.body.map((curentval) => {
        console.log(curentval, "curentval");
        let fullName = `${curentval?.first_name} ${curentval?.middle_name} ${curentval?.last_name}`;
  ...
  //   // Access the doctor's name parts from the selectedDoctor object
  //   const doctorFirstName = selectedDoctor?.first_name;
  //   const doctorMiddleName = selectedDoctor?.middle_name;
  //   const doctorLastName = selectedDoctor?.last_name;

  //   this.fourthFormGroup.patchValue({
  //     prescriberFName: doctorFirstName,
  //     prescriberLName: doctorLastName,
  //     prescriberMName: doctorMiddleName
  //   });

  // }

  onPrescriberDoctorList(selectedValue: any) {
    console.log("selectedValue>>>>>>>>", selectedValue)
    // let doctorNamedata = selectedValue.split("-");
    let doctorNamedata = selectedValue;

    this.fourthFormGroup.patchValue({
      // prescriberFName: doctorNamedata[0],
      // prescriberLName: doctorNamedata[2],
      // prescriberMName: doctorNamedata[1]
      prescriberFName: doctorNamedata?.first_name,
      prescriberLName: doctorNamedata?.last_name,
      prescriberMName: doctorNamedata?.middle_name
    });
    const selectedDoctor = this.doctorList.find(doctor => {
    ...
    if (selectedDoctor) {
      const doctorId = selectedDoctor.for_portal_user._id;
  ...
  getAllHospitalList() {
    ...
    this.patientService.getfourportalClinicHospital(reqData).subscribe((res) => {
      let response = this.coreService.decryptObjectData({ data: res });
      console.log("Hospital RESPONSE LIST====>", response);
      this.locationList12 = response?.body;
      this.locationListAllData = [];
      this.locationList12.map((curentval, index) => {
        this.locationListAllData.push({
          value: curentval.for_location_info?.hospital_name,
          label: curentval.for_location_info?.hospital_name,
        });
      });
  ...
  public getAppointmentDetails(): void {
    ...
    this.labImaginge.fourPortal_appointment_deatils(reqData).subscribe({
      next: (result1: any) => {
        ...
        console.log(result, "check result order");
        this.orderDetails = result?.data
        console.log(this.orderDetails, "check order details");


        if (result.status === true) {
          setTimeout(() => {
            // this.deliveryCenterselected = this.pharmacyIdOrder;
            // this.locationId = 
            // this.onPrescriberCenterChange(result?.data?.appointmentDetails?.hospital_details?.hospital_id)
            this.getFourPortalAsperlocation(result?.data?.appointmentDetails?.hospital_details?.hospital_id)
            this.deliverTitleselected = result?.data?.doctor_basic_info?.basic_info?.title;
            this.patientSelectedId = result?.data?.patientAllDetails?.insuranceDetails?.subscriber_id;
            this.doctorListselectedname = result?.data?.doctor_basic_info?.full_name;
            this.prescriberCenter = result?.data?.appointmentDetails?.hospital_details?.hospital_name
            this.doctorListselected = result?.data?.doctor_basic_info?.full_name;
            this.prescriberSpecialitydefault = result?.data?.doctor_basic_info?.specialityPromises;
            console.log(this.deliveryCenterselected, "123444", this.subscriberId2, "000000", result?.data?.patientAllDetails?.portalUserDetails?._id);
            this.getPatientDetails(result?.data?.patientAllDetails?.portalUserDetails?._id);
          }, 2000);
        }
      },
      error: (err: ErrorEvent) => {
