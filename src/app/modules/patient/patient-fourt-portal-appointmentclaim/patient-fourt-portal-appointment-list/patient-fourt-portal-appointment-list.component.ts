...
@Component({
  selector: 'app-patient-fourt-portal-appointment-list',
  templateUrl: './patient-fourt-portal-appointment-list.component.html',
  styleUrls: ['./patient-fourt-portal-appointment-list.component.scss'],
...
export class PatientFourtPortalAppointmentListComponent implements OnInit {
  ...
  getStatusCount() {
    this.patientSerivice
      .medicineClaimCountByStatusLabImaginAppointmentPatient(this.patientId, this.loggedType + "-appointment")
      .subscribe((res) => {
        let response = this.coreService.decryptObjectData({ data: res });
  ...
  editclaim(claim_id, type = '', preAuthReclaimId = '') {
    console.log("preauth", preAuthReclaimId)
    if (preAuthReclaimId != '') {
      type = "pre-auth";
      claim_id = preAuthReclaimId
    }
    this.router.navigate([`/patient/appointment-claim/submitclaim/${this.loggedType}`], {
  ...
  goToNewEdit(claimId) {
    this.router.navigate([`/patient/appointment-claim/details/${claimId}/${this.loggedType}`])
  }
  exportLab() {
