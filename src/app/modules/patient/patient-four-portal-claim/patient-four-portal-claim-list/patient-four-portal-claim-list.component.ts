...
@Component({
  selector: 'app-patient-four-portal-claim-list',
  templateUrl: './patient-four-portal-claim-list.component.html',
  styleUrls: ['./patient-four-portal-claim-list.component.scss'],
...
export class PatientFourPortalClaimListComponent implements OnInit {
  ...
  editclaim(claim_id, type = '', preAuthReclaimId = '') {
    ...
    this.router.navigate([`/patient/order-claim/submitclaim/${this.loggedType}`], {
  ...
  goToNewEdit(claimId) {
    this.router.navigate([`/patient/order-claim/details/${claimId}/${this.loggedType}`])
  }
    ...
    this.patientSerivice.medicineClaimListLabImaging(reqData).subscribe((res) => {
      let response = this.coreService.decryptObjectData({ data: res });
        ...
        if(response.status == true){
          ...
          var fileName = 'patientOrderExcel.xlsx';
