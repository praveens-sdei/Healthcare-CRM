          ...
          if (claimData?.deliverCenterInfo?.deliverTitleId != undefined) {
            ...
            this.pharmacyService.getPharmacyStaffName(param).subscribe({
              next: async (res) => {
                ...
                console.log(result, "dddddddd", this.staffRole);
                if (result.status) {
                  // if(this.staffRole != ""){
                  ...
                  console.log(claimData?.deliverCenterInfo?.deliverFirstName, "hhhhhhhh");
  ...
  getSpecialty() {
    ...
    this._superAdminService.listSpeciality(param).subscribe({
      next: (res) => {
        ...
        console.log(this.specialtyList, "check dataall");
      },
      error: (err) => {
        console.log(err);
      },
          ...
          if (response.body.subscriberId == "") {
            ...
            if (response.status) {
              ...
              this.prescriberSpecialitydefault =
                response?.body?.prescriberCenterDetails?.prescriberSpeciality;
              this.deliverTitleselected = response.body?.deliverCenterInfo?.deliverTitleId;
            } else {
              this.ePrescNumber = "";
      ...
      next: (result1: any) => {
        ...
        if (result.status === true) {
          ...
          this.firstFormGroup.patchValue({
            ...
            patientId: result?.data?.orderData?.subscriber_id,
            claimNumber: '',
          });
