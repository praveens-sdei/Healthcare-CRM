...
import { Select2Data, Select2UpdateEvent, Select2SearchEvent, Select2UpdateValue } from "ng-select2-component";
...
export class ServicetypeComponent implements OnInit {
    ...
    categoryLimit: any = [];
    dataofJoining: string | number | Date;
    deleteIndex: any;
    deleted_id: any;
    famiyTotalLimit: any;
    ...
    constructor(
            ...
            (res) => {
                if (res != "default") {
                    ...
                    this.route.queryParams.subscribe((params: any) => {
                        ...
                        this.pharmacyIdOrder = params.pharmacy_id
                    });
        ...
        if (this.stepOneId == null || this.insuranceId == null) {
            this.router.createUrlTree(["/pateint/medicinclaims"]);
        }
                ...
                .subscribe({
                    next: async (res) => {
                        ...
                        if (claimData?.medicinedetailsonclaims.length > 0) {
                            claimData?.medicinedetailsonclaims.forEach(
                                (element: any, index: any) => {
                                    this.addNewMedicine(element.medicineName, element.medicineId, element.serviceName, element.categoryService);
                                    console.log("addNewMedicine5");
                                    this.selectedMedicine[index].name = element.medicineName;
                                    obj.push({
                                        date_of_service: element.date_of_service,
                                        categoryService: element.categoryService,
    ...
    patchEprescriptionMedicines(medicines) {
        ...
        if (medicines?.length > 0) {
            medicines.forEach((element: any, index: any) => {
                ...
                let frequencyStatements = [];

                if (frequency?.frequency?.frequency_type === "Moment") {
                    let momentStatement = `Morning(${frequency?.frequency?.morning}), Midday(${frequency?.frequency?.midday}), Evening(${frequency?.frequency?.evening}), Night(${frequency?.frequency?.night}) for ${frequency?.take_for?.quantity} ${frequency?.take_for?.type}`;
                    obj.push({ frequency: momentStatement });
                    frequencyStatements.push(momentStatement);
                }

                if (frequency?.frequency?.frequency_type === "Recurrence" || frequency?.frequency?.frequency_type === "Alternate_Taking") {
                    let recurrenceStatement = `Medicines(${frequency?.frequency?.medicine_quantity}) for every ${frequency?.frequency?.every_quantity} ${frequency?.frequency?.type}, ${frequency?.take_for?.quantity} ${frequency?.take_for?.type}`;
                    obj.push({ frequency: recurrenceStatement });
                    frequencyStatements.push(recurrenceStatement);
                }

                obj.push({
                    frequency: frequencyStatements,
    ...
    search(event: Select2SearchEvent<Select2UpdateValue>) {
        const searchValue = event;
        console.log("Search value:", searchValue);
        // Now you can use the search value as needed in your component logic
    }
    ...
    async addMedicineForSuperadmin(index) {
        return new Promise((resolve, reject) => {
            ...
            medicineArray.push({
                medicine: {
                    number: "",
                    medicine_name: this.selectKeyPressValue[index],
                    inn: "",
                    dosage: "",
            ...
            if (medicineArray.length > 0) {
                ...
                this.superAdminService.addMedicine(reqData).subscribe(async (res: any) => {
                    let response = this.coreService.decryptObjectData({ data: res });
                    console.log(response, 'medicine added successfully');

                    if (response.status) {

                        await this.getMedicineList(this.selectKeyPressValue[index], index);
                        setTimeout(() => {
                            console.log(this.actualMedicineList[index][0], "this.medicineList[index]");
                            if (this.actualMedicineList[index].length > 0) {
                                var medicineValue = this.actualMedicineList[index][0];
                                console.log(medicineValue, "check medicine value");

                                this.medicineIDObject[index] = medicineValue._id;
                                this.medicines.at(index).patchValue(medicineValue._id);
                            }
    ...
    onSubmit() {
        ...
        if (allSame) {
            ...
            let reqData = {
                ...
                pharmacyId: "",
                loggedInPatientId: this.loggedId,
    ...
    getMedicineList(query: any = "", index: any = 0, medicneId: string = "", serviceName: string = '', categoryService: string = '') {
        ...
        this.patientService.getmedicineListWithParam(param).subscribe(
            (res) => {
                ...
                const medicineArray = [];
                this.actualMedicineList[index] = result.body.medicneArray
    ...
    getPortalTypeAndInsuranceId() {
        ...
        console.log(data, "check data 444");
    ...
    public getOrderDetails(): void {
        const orderDetailRequest: any = {
            for_portal_user: this.pharmacyIdOrder,
        ...
        this.patientService.getOrderDetails(orderDetailRequest).subscribe({
            next: (result1: any) => {
                ...
                if (result.status === true) {
                    ...
                    if (result?.data?.medicineDetails.length > 0) {
                        result?.data?.medicineDetails.forEach(
                            (element: any, index: any) => {
