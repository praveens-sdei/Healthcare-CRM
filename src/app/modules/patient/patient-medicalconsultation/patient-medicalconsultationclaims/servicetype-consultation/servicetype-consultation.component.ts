...
export class ServicetypeConsultationComponent implements OnInit {
  ...
  constructor(
    ...
    this.ePrescriptionData =
      this.coreService.SharingEprescriptionData.subscribe(async (res: any) => {

        if (res != "default") {
          console.log(res.reasonForAppointment, "check number");

          let reasonForAppointment = res?.reasonForAppointment;
          let medicines: any = res?.medicines;
          this.ePrescriptionMedicines = res?.medicines;
          console.log(this.ePrescriptionMedicines, "check number");
          this.resonForAppoinment(this.ePrescriptionMedicines[0].doctorId, res.reasonForAppointment)
  ...
  resonForAppoinment(val: any, reasonForApp) {

    // this.doctor_portal_id = [];

    let param = {
      doctorId: val,
      // "hospitalId":  this.location_id
      searchText: "",
      limit: 0,
      page: 1,
    };

    this.doctorService.resonForAppoinment(param).subscribe({
      next: (res) => {
        let result = this.coreService.decryptObjectData({ data: res });
        // this.resonForAppoinmentList = result?.body?.data
        console.log("reson", result.body.data)
        this.reasonArray = result.body.data;
        this.getRecordByCustomId(reasonForApp)
      },
      error: (err) => {
        console.log(err);
      },
  ...
  getRecordByCustomId(customId) {
    return new Promise(resolve => {
      console.log(this.reasonArray, "45454", customId);

      this.matchedReason = this.reasonArray.find(record => record._id === customId);
      console.log(this.matchedReason.name, "check record match2");
      let obj = [];
      obj.push({
        // quantity_prescribed: element?.quantities?.quantity,
        // duration: element?.take_for?.quantity,
        // frequency: frequency,
        reasonOfConsultation: this.matchedReason.name
      });
      resolve(this.matchedReason);
    });
  }
    ...
    if (medicines?.length > 0) {
      medicines.forEach((element: any, index: any) => {
        ...
        obj.push({
          ...
          duration: element?.take_for?.quantity,
          frequency: frequency,
          reasonForAppointment: this.matchedReason.name
        });
      });
    } else {
      this.addNewMedicine();
  ...
  onSubmit() {
    ...
    if (allSame) {
      ...
      this.medicineForm.value.medicines.map((data, index) => {
        ...
        let reasonOfConsultationValue = data.reasonOfConsultation === 'Other' ? data.otherReason : data.reasonOfConsultation;
        medicineArray.push({
          indexNumber: index,
          ...
          reasonOfConsultation: reasonOfConsultationValue,
  ...
  getAllFourPortalappointmentReason() {
    let reqData = {
      page: this.page,
      limit: this.pageSize,
      searchText: this.searchText,
      loginPortalId: localStorage.getItem('selected-doctor-id'),
      listFor: this.selectedPortalType,
      selectedlocation: localStorage.getItem('selected-location-id')
    };
    this.labImaginge.listAppointmentReason(reqData).subscribe((res) => {
      ...
      //   this.appointmentReasonArray.push({
  ...
  async calCulatePrice(event: any, i: number) {
    ...
    if (this.serviceObject[i] != undefined && this.serviceObject[i].length > 0) {
      ...
      let prcPerunit = this.medicineForm.value.medicines[i].reasonOfConsultation;
  ...
  getCategoryId(event: any, i: any) {
    if (event.value != undefined) {
      ...
      this.serviceCode[i] = "";
      console.log(event.value, "event.value opopo");
