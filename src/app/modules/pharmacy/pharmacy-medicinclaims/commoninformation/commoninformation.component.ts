...
export class CommoninformationComponent implements OnInit {
  ...
  constructor(
    ...
    let user = this.coreService.getLocalStorage("loginData");
    let adminData = JSON.parse(localStorage.getItem("adminData"));
    this.loggedRole = this.coreService.getLocalStorage("loginData")?.role;

    this.userRole = user.role;
    this.loggedInProfileId = user?._id;
    console.log(this.loggedInProfileId, "check progile id");

    if (this.userRole === "PHARMACY_STAFF") {
      this.loggedId = adminData?.for_staff;
    } else {
      this.loggedId = user?._id;
    }
  ...
  ngOnInit() {
    ...
    this.getstaffdetailsProfile(this.loggedInProfileId);
    this.getAllRole();
  ...
  getSpecialty() {
    ...
    this._superAdminService.listSpeciality(param).subscribe({
      next: (res) => {
        ...
        console.log(result.body.data, "check result data");
        ...
        console.log(this.specialtyList, "specialtyList")
      },
      error: (err) => {
        console.log(err);
      },
  ...
  getstaffdetailsProfile(id: any) {
    ...
    this._pharmacyService.getStaffDetails(id).subscribe({
      next: (res) => {
        let result = this.coreService.decryptObjectData({ data: res });
        this.staff_detailsProfile = result?.data?.data;
        this.staffDetailsRole = this.staff_detailsProfile?.staffInfo[0]?.role?._id;
        // this.selectedLanguages = this.staff_details?.in_profile?.language;
        console.log(this.staff_detailsProfile, "staff_detailsProfile,", result);
        if (this.loggedRole === "PHARMACY_STAFF") {
    ...
    this.pharmacyService.getPharmacyStaffName(param).subscribe({
      next: async (res) => {
        ...
        if (result.status) {
          ...
          if (this.loggedRole === "PHARMACY_STAFF" && role != '') {
            this.deliverStaffselected = this.staff_detailsProfile?.staffInfo[0]?.staff_name;
          }
