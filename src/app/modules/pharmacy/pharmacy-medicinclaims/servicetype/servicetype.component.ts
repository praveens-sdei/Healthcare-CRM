...
export class ServicetypeComponent implements OnInit {
  ...
  serviceLimit: any = [];
  categoryLimit: any = [];
  familysubscriber: any = [];
  individualsubscriber: any = [];
  ...
  update(key: string, event: Select2UpdateEvent<any>, index: any) {
    if (event.component.option != null) {
      ...
      console.log(this.medicineIDObject[index], "this.medicineIDObject[index]123", event)
  ...
  getMedicineList(query: any = "", index: any = 0, medicneId: string = "", serviceName: string = '', categoryService: string = '', serviceCode: string = '') {
    ...
    this.patientService.getmedicineListWithParam(param).subscribe(
      (res) => {
        ...
        var medicineArray = [];
        console.log("result-------------", result.body.medicneArray);
    ...
    if (this.insuranceId != null) {
      this.pharmacySerivice.getPortalTypeAndInsuranceId(data).subscribe({
        next: (res) => {
          ...
          this.categoryData.forEach((currentVal: any) => {
            let checkCategoryData = this.categoryList.some(obj => obj.label === currentVal);
          ...
          if (result?.data?.medicineDetails.length > 0) {
            result?.data?.medicineDetails.forEach(
              (element: any, index: any) => {
                ...
                // if (this.type != '') {
                console.log("addNewMedicine1", element);
  ...
  addMedicineForSuperadmin(index) {
    return new Promise((resolve, reject) => {
      const medicineArray = [];
