...
var lable=event.options[0].label.toLowerCase()
  ...
  async calCulatePrice(event: any, i: number) {
    ...
    if (this.health_plan_details?.planExclusion) {
      ...
      var medicinename=this.medicineNameObject[i].toLowerCase();
  ...
  onValueChnage() {
    ...
    this.orderMedicine.patchValue({
      copay: this.orderDetails?.medicineBill?.total_medicine_cost,
      insurance_paid: this.orderDetails?.medicineBill?.insurance_paid,
      total_cost: this.orderDetails?.medicineBill?.co_pay
      ,
    });


    console.log(">>>>>>>>>>>>>>>>>>>", this.orderMedicine.value);

  }
  public updateMySelection(event: any, option: any, index: any) {
    if (event.isUserInput) {
      this.medicineIDObject[index] = option.medicine_id
      this.medicineNameObject[index] = option.medicine_name
      const value = this.getOrderField("medicine_details")[index].totalcost
      if (this.health_plan_details) {
        this.handleMedicineExclusionCost(value ? value : 0, index)
      }
    }
  }
  ...
  handleMedicineExclusionCost(price: any, index: any): void {
  ...
  // public setData() {
  //   const allOrderDetails = this.getOrderField("medicine_details")
  //   for (const index in allOrderDetails) {
  //     this.medicineIDObject[index] = allOrderDetails[index].name
  //     this.medicineNameObject[index] = this.orderDetails.medicineNameObject[allOrderDetails[index].name]
  //     this.handleMedicineExclusionCost(allOrderDetails[index].totalcost, index)
  //   }
  // }
  ...
  async calculateMedicineCost(index: any) {
    const currentData = this.getOrderField("medicine_details")[index]
    ...
    await this.medicineData.controls[index].patchValue({
      totalcost: totalIndividualCost,
    });
    this.handleMedicineExclusionCost(totalIndividualCost, index)
    this.calculateCost()
  }
