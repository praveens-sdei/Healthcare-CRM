...
@Component({
  selector: 'app-commoninformation-hospital',
  templateUrl: './commoninformation-hospital.component.html',
  styleUrls: ['./commoninformation-hospital.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class CommoninformationHospitalComponent implements OnInit {
  ...
  fourthFormGroup = this._formBuilder.group({
    ...
    hospitalCenter: new FormControl("", [Validators.required]),
  ...
  constructor(
    ...
    this.adminData = this.coreService.getLocalStorage("adminData").for_hospitalIds;
    ...
    if (this.loggedRole == "INDIVIDUAL_DOCTOR_STAFF" || this.loggedRole == "HOSPITAL_DOCTOR_STAFF") {
      this.loggedId = this.coreService.getLocalStorage("adminData").in_hospital;
      this.loggedName =
        this.coreService.getLocalStorage("adminData").adminName;
    ...
    this.pharmacyService.medicineClaimDetailsPharmacyClaimObjectIdHopitalization(this.selectclaimid).subscribe({
      next: async (res) => {
        ...
        if (result.status) {
          ...
          this.patientSelectedId = claimData?.patientId;
            ...
            this.indivudualDoctorService.getAllStaff(params).subscribe({
              next: async (result: IEncryptedResponse<SuperAdminStaffResponse>) => {
                ...
                if (decryptedData.status) {
                  ...
                  this.deliverStaffselected = claimData?.deliverCenterInfo?.deliverFirstName;
  ...
  private getAllRole() {
    this._pharmacyService.allRole(this.loggedIdProfile).subscribe((res) => {
      ...
      if (result?.status) {
        ...
        //   const foundObject = data.find((obj) => obj._id === this.staffDetailsRole);
        //   if (foundObject) {
        //     if (!Array.isArray(this.staffRole)) {
        //       this.staffRole = []; // Initialize as an array if it's not already
        //     }
        //     this.staffRole.push(foundObject);
        //   } else {
        //     // Handle case where no object with matching _id is found
        //   }
        // } else {
  ...
  getStaffDetails(event: any, role = '') {
    ...
    this.indivudualDoctorService.getAllStaff(params).subscribe({
      next: async (result: IEncryptedResponse<SuperAdminStaffResponse>) => {
        ...
        console.log("getAllStaff", decryptedData, "result", result);
        ...
        data.forEach((currentVal) => {
          this.deliverStaff.push({
            label: currentVal?.staffname,
            value: currentVal?.staffname,
          });
        });

        if (this.loggedRole === "INDIVIDUAL_DOCTOR_STAFF" && role != '') {
  ...
  //   let hospitalId = filterObject[0]?.hospital_id;
