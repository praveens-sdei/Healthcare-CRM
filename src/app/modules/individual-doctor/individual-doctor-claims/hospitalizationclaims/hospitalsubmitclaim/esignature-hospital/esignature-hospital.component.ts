...
@Component({
  selector: 'app-esignature-hospital',
  templateUrl: './esignature-hospital.component.html',
  styleUrls: ['./esignature-hospital.component.scss']
})
export class EsignatureHospitalComponent implements OnInit {
  ...
  submitClaim() {
    ...
    const data = {
      ...
      preAuthReclaimId: this.selectclaimid
    ...
    this.pharmacyPlanService.finalSubmitClaimDoctor(data).subscribe({
      next: async (res) => {
        ...
        if (result.status) {
          ...
          this.closePopUp();
          this.router.navigate(["/individual-doctor/dashboard"]);
        } else {
