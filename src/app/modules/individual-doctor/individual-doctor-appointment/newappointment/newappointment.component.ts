...
export class NewappointmentComponent implements OnInit {
  ...
  async doctorAppointment() {
    ...
    if (this.consultationFee === undefined || this.consultationFee === null) {
      this.toster.error("Doctor don't have consultation fees. Please Check!");
      return
    } else {
  ...
  public onSelection(data: any) {
    ...
    console.log("this.chooseSlot",this.chooseSlot)
    ...
    console.log("this.selectelocationId>>>>>>>>",this.selectelocationId)
    this.resonForAppoinment();
    this.doctorAvailableSlot();
  }
    ...
    this._IndiviualDoctorService.doctorAvailableSlot(param).subscribe({
      next: async (res) => {
        ...
        if (result.status) {
          ...
          console.log("doctorAvailableTimeSlot", this.doctorAvailableTimeSlot);
          ...
          console.log("doctorAvailableTimeSlot", result?.body);
  ...
  SubscribersList(patient: any) {
    ...
    console.log("Selected Patient Details===>", this.reasonSelectedId
    );
    let patientdata: any;
    let param: any;
    if (patient?.options === null) {
      ...
      this._PatientService.SubscribersList(param).subscribe({
        next: (res) => {
          let result = this._CoreService.decryptObjectData({ data: res });
          console.log("PATIENT SUBSCRIBER LIST====>", result);
  ...
  handelSubscribPatient(user: any) {
    console.log("userrrrr", user.value)
    if (user.value === " ") {
      this.patient_details.reset();
    } else {
  ...
  createMedicalDoc() {
    ...
    this._PatientService.createMedicalDoc(param).subscribe({
      next: (res) => {
        ...
        console.log("PATIENT DOCS===>", result);
    ...
    this._PatientService
      ...
      .subscribe({
        next: (res) => {
          ...
          console.log("PATIENT EXISTING DOCUMENTS===> ", result);
        },
        error: (err) => {
          console.log(err);
        },
  ...
  getDependentFamilyMembers(isFirstTime: any = "") {
    console.log("SELECTED PATIENT===>", this.selectedPatientDetails);
  ...
  navigateToPage(){
    this.router.navigate([`/individual-doctor/patientmanagement/add`])
  }
  geoAddress(){
    const options = {
      fields: [
