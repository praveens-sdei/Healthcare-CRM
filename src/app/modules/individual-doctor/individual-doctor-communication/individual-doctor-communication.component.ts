...
export class IndividualDoctorCommunicationComponent implements OnInit {
  ...
  constructor(
    ...
    // if(userData?.role === "INDIVIDUAL_DOCTOR"){
    //   this.adminId = adminData?.for_hospitalIds[0];
    // }else{
    //   this.adminId=adminData?.in_hospital
    // }
      ...
      next: (result: IEncryptedResponse<SuperAdminStaffResponse>) => {
        ...
        if (staffDetails?.body?.length > 0) {
          for (const staff1 of staffDetails?.body) {
            ...
            //   console.log("runnnnnnnnnn11111")
            //   for (const ele of staff?.staffinfos) {
            //     console.log("staffffffff", ele)
            //     data.push({
            //       staffname: ele?.name,
            //       staffImage: ele?.profile_pic ? ele?.profile_pic : '../../../../assets/img/default_user.png',
  ...
  handleroute(data: any, openDiv: boolean) {
    ...
    // let checkOnline=(this.userID === data?.senderDetails?._id) ? data?.receiverDetails[0]?.isOnline : data?.senderDetails?.isOnline;

    // this.isOnline = data?.receiverDetails[0].isOnline;
    ...
    if (data.isGroupChat) {
      // this.selectedrecieverimage = "../../../../assets/img/GroupIcon.png"
      ...
      // this.selectedrecieverimage = data?.receiverDocumentImage[0]?.url ? data?.receiverDocumentImage[0]?.url : "../../../../assets/img/default_user.png"
    }
  ...
  previewImage(path: any, type?: any) {
    ...
    this.showLightbox(0);
    // let obj = { image: path, title: title };
    // console.log("objjjjjjjjjj", obj.image)
    // // this.imageObject.splice(0, this.imageObject.length);
    // // console.log("this.imageObject", this.imageObject.length)
    // this.imageObject.push(obj.image);
    // this.showLightbox(0);
  }
