...
export class EditprofileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // view_profile_Data
    if (this.doctorRole === "HOSPITAL_STAFF" || this.doctorRole === 'INDIVIDUAL_DOCTOR_STAFF') {
  ...
  /* Staff Edit profile */
  handleEditProfile() {
    if (this.doctorRole == "INDIVIDUAL_DOCTOR" || this.doctorRole == "HOSPITAL_DOCTOR") {
      this.router.navigate(["/individual-doctor/myprofile/0"])

    } else if (this.doctorRole == "HOSPITAL_STAFF" || this.doctorRole == "INDIVIDUAL_DOCTOR_STAFF") {
      let loginData = JSON.parse(localStorage.getItem("loginData"));
  ...
  keyURL: any = "";
  uploadDocument() {
    this.indiviualDoctorService.uploadDoc(this.staff_profile_file).subscribe({
      next: async (res: any) => {
  ...
  getstaffdetails(id: any) {
    ...
    this.indiviualDoctorService.getStaffDetails(pararm).subscribe({
      next: (res) => {
        ...
        console.log(this.selectedLanguages,"staff_detailss____");
  ...
  getDoctorDetails() {
    this.service.getDoctorProfileDetails(this.doctorId).subscribe((res) => {
      ...
      if (response.status == true) {
        ...
        // this.getTeamList(response?.data?.result[0]?.team)
  ...
  handleSelectConsulation(event: any) {  
    if(this.selected_hospitalLocation[0] !== undefined){
      ...
      console.log("seleettet",this.selectedData)
    }
  }
