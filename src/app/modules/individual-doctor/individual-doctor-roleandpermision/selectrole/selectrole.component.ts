    ...
    this.doctorService.getAllStaffWithoutPagination(params).subscribe((res) => {
      ...
      if (response.status) {
        ...
        response?.body.map((curentval, index: any) => {
          console.log("curentval?.profileinfos?.for_portal_user",curentval?.profileinfos?.for_portal_user)
    ...
    this.doctorService.getUserMenus(params).subscribe(
     async (res: any) => {
        ...
        for (const data of decryptedData.body) {
          if (data.menu_id._id != "63e2870750fa3f54d75ef4a4" && data.menu_id._id != "646ee6735c80001a548c42f8") {
            if (data.parent_id === "") {
              const menuobj = {};
