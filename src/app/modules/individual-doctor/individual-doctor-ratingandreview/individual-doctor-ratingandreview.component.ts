...
export class IndividualDoctorRatingandreviewComponent implements OnInit {
  ...
  constructor(
    ...
    this.for_hospitalIds = adminData?.for_hospitalIds
      ...
      // //   this.docotrId = adminData?.for_doctor;
  
      // }else 
      if(this.userRole === "INDIVIDUAL_DOCTOR_STAFF"){
        console.log();
        
        this.docotrId = adminData?.in_hospital;
      }else{
  ...
  postRatingAndReviwe() {
    ...
    let pararm = {
      ...
      rating: this.hospitalRatingForm.value.rating,
      comment: this.hospitalRatingForm.value.comment,
      reviewBy: "doctor",
      reviewTo:'hospital'
    };
    ...
    this.service.DoctorReviweandRating(pararm).subscribe({
      next: (res) => {
